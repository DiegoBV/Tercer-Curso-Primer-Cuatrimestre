...
protected:
	int k_Element(int k, Link& node) {
		...
		if (k > node->tam_i) {
			return k_Element(k - node->tam_i, (node->dr));
		}
		else if (k < node->tam_i) {
			return k_Element(k, (node->iz));
		}
	}
...
void resuelve(map<int, int>& tree, std::queue<int>& q);
void entrada();
