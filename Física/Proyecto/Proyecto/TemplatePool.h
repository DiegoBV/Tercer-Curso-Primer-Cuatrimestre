...
public:
	TemplatePool() { for (int i = 0; i < DEFAULT_NUMB; i++) { T* aux = new T(new RenderItem()); 
		v.push_back(aux); aux->setActive(false); } };                               //crea un numero inicial por defecto

	TemplatePool(const unsigned int numInicial) { for (int i = 0; i < numInicial; i++) { T* aux = new T(new RenderItem()); 
		v.push_back(aux); aux->setActive(false); } };                               //crea un numero proporcionado 
	...
	T* generateObject(Vector3 pos, Vector3 vel, Vector3 ac, Vector4 color) {
