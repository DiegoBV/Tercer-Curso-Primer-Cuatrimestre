...
void SpringManager::addParticle_to_Liquid(Vector3 pos, float _maxDepth, float _volume, float _waterHeight, float _liquidDensity)
{
	...
	ParticleBuoyancy* b = new ParticleBuoyancy(_maxDepth, _volume, _waterHeight, _liquidDensity);
	springs.push_back(b);
	...
	particles.push_back(new Particle(new RenderItem(), 0));
